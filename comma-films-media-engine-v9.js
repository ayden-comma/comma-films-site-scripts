/* ============================================================
   Comma Films — home-new media engine v9
   v9 changes:
...
(() => {
  ...
  .csvd_nav{position:fixed;top:50%;transform:translateY(-50%);width:52px;height:52px;border-radius:50%;border:1px solid #ffffff38;background:#0c0c0e6b;cursor:pointer;display:grid;place-items:center;z-index:1000000;transition:background .25s,border-color .25s}
  .csvd_nav:hover{background:rgb(232,90,79);border-color:rgb(232,90,79)}
  .csvd_nav:hover svg{stroke:#fff}
  .csvd_nav svg{width:20px;height:20px;stroke:rgb(232,90,79);stroke-width:2;fill:none;stroke-linecap:round;stroke-linejoin:round;transition:stroke .25s}
  ...
  @media(max-width:600px){.csvd_nav{width:46px;height:46px;top:auto;bottom:22px;transform:none}.csvd_prev{left:20px}.csvd_next{right:20px}.csvd_cap{bottom:80px}}
  /* thumbnail-strip circular arrows (Webflow's own) -> red hover to match */
  .story_slider-wrap-main .splide__arrow:hover,.story_slider-wrap-main [class*="arrow"]:hover{background:rgb(232,90,79)!important;border-color:rgb(232,90,79)!important}
  .story_slider-wrap-main .splide__arrow:hover svg,.story_slider-wrap-main [class*="arrow"]:hover svg path{stroke:#fff!important}
  /* mobile popup chrome */
  @media(max-width:767px){
  ...
  function openDialog(index, autoplay) {
    ...
    // swipe left/right to step (mobile primary control; arrows secondary)
    let sx = 0, sy = 0, swiping = false;
    d.addEventListener('touchstart', ev => { const t = ev.touches[0]; sx = t.clientX; sy = t.clientY; swiping = true; }, { passive: true });
    d.addEventListener('touchend', ev => {
      ...
      if (Math.abs(dx) > 45 && Math.abs(dx) > Math.abs(dy)) { stepDialog(dx < 0 ? 1 : -1); }
    }, { passive: true });

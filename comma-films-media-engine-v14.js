/* ============================================================
   Comma Films — home-new media engine v14
   v14 changes:
...
(() => {
  ...
  .csvd_strip{display:flex;gap:16px;width:100vw;overflow-x:auto;scroll-snap-type:x mandatory;-webkit-overflow-scrolling:touch;scrollbar-width:none;padding:0 30vw;align-items:center}
  .csvd_strip::-webkit-scrollbar{display:none}
  .csvd_cell{flex:0 0 40vw;scroll-snap-align:center;height:auto;aspect-ratio:16/10;border-radius:12px;overflow:hidden;background:#0e0c0a;position:relative;display:flex;align-items:center;justify-content:center;opacity:.45;transition:opacity .3s;box-shadow:0 0 0 1px #ffffff10}
  ...
  const buildStepper = (wrap, items) => {
    ...
    galleries.set(wrap, { teardown: () => { strip.removeEventListener('scroll', onScroll); dots.removeEventListener('click', onDot); } });
  };

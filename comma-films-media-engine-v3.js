/* ============================================================
   Comma Films — home-new media engine v3
   v3 changes:
...
(() => {
  ...
  const mountSplide = (el, count) => {
    ...
    if (sliders.has(el)) { try { sliders.get(el).refresh(); } catch (e) {} return; }
    const sp = new Splide(el, {
      type: 'slide', rewind: true, perPage: Math.min(2, count || 1), perMove: 1, focus: 0, gap: '14px',
  ...
  const buildVisibleGallery = () => {
    ...
    const splideRoot = m.querySelector('.story_slider-wrap-main .splide') || m.querySelector('.splide');
    if (!splideRoot) return;
    if (!items.length) { console.warn('[cf-gallery] no valid items for', slugOf(m)); return; }
    // destroy any prior instance so we can repopulate
    if (sliders.has(splideRoot)) { try { sliders.get(splideRoot).destroy(true); } catch (e) {} sliders.delete(splideRoot); }
    buildGallery(splideRoot, items);
    m.classList.add('cf-gallery-built');
    const wrap = m.querySelector('.story_slider-wrap-main');
    if (wrap) wrap.classList.add('cf-gallery-built');

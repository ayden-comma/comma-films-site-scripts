/* ============================================================
   Comma Films — home-new media engine v12
   v12 changes:
...
(() => {
  ...
  @media(max-width:600px){.csvd_swipe{display:block;position:fixed;left:0;right:0;top:calc(50% - 26vw);height:52vw;z-index:999999}}
  ...
  @media(max-width:600px){
    .csvd{align-items:center;justify-content:center;padding:0}
    .csvd_b{margin:0;width:92vw}
    .csvd_x{top:max(14px,env(safe-area-inset-top));right:16px}
    /* arrows sit BELOW the media, on the caption line (media is 92vw wide => ~51.75vw tall at 16:9, centered) */
    .csvd_nav{top:calc(50% + 27vw);bottom:auto;transform:none;width:40px;height:40px}
    .csvd_nav:hover{transform:none}
    .csvd_prev{left:14px}.csvd_next{right:14px}
    /* caption centered between the two arrows, same line */
    .csvd_cap{position:absolute;left:66px;right:66px;bottom:auto;top:calc(50% + 27vw + 8px);text-align:center;line-height:40px}
  }
  /* touch devices: no sticky hover. Neutral at rest; red only while finger is down (:active). */
  @media(hover:none){

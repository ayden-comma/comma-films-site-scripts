/* ============================================================
   Comma Films — home-new media engine v28
   v28 changes:
   - Mobile hero stepper: cells full-width (100%, track padding 0) — no side
     peek; the red dots signal that more slides exist. Desktop unchanged
     (86% + 6% peek). Bottom gallery peek untouched on both.
   ---- v27 changes:
   - HERO GALLERY: new per-case-study [cf-hero-data] field ("image :: url",
     image-only, captions ignored). If the popup has NO real hero video
     (no vimeoMap entry AND no iframe/video with a non-empty src/data-src in
     .case-study_video / .story_video-wrap — a video ALWAYS wins), the video
     embed is hidden, .story_video-wrap gets class cf-hero-built, and the
     standard stepper is built into it with captions suppressed
     ({noCaption:true}). Cells 86% (94% mobile), 16/9; track padding 6% (3%
     mobile). Init order: hero, gallery, credits, meta.
   - LIGHTBOX ISOLATION: openDialog(items, index, autoplay) is now a fully
     self-contained closure — per-instance items/track/pos/anim/CLONES and
     per-instance place/sync/go/goTo/close. Open dialogs register on lbStack
     (last = topmost) and expose d._lb = {go,goTo,close}. Hero and bottom
     galleries share NO state and can coexist. closeDialog() closes the
     TOPMOST dialog; Escape/arrows drive the topmost. Module globals
     currentGalleryItems/lbTrack/lbPos/lbAnim/lbClones and legacy
     stepDialog/renderDialogItem removed. Closing a lightbox NEVER touches
     the body scroll lock — the case-study popup beneath still owns it
     (closePopup is the only unlocker).
   - Removed the dead legacy ".cf-gal-slide.is-media" click branch (cells
     have been .cf-gal-cell since the v17 stepper; nothing emits it).
   - FIX (pre-existing v26 bug): buildStepper's fixed CLONES=2 crashed any
     gallery with exactly ONE item (pushed items[1]=undefined into makeCell,
     aborting credits/meta for that popup). Now Math.min(2, N) — matters for
     single-image hero galleries.
   ---- v16 changes:
...
(() => {
  ...
  @media(min-width:768px){
    .work-item.new-des video.video-bg{display:block;visibility:visible;opacity:0;z-index:3;pointer-events:none;transition:opacity .6s ease}
    .work-item.new-des img.reel{display:block;visibility:visible;opacity:1;z-index:4;transition:opacity .6s ease}
  ...
  [cf-credits-data]{display:none!important}
  [cf-meta-data]{display:none!important}
  [cf-hero-data]{display:none!important}
  .story_item-info-wrap.cf-meta-empty{display:none!important}
  /* rendered credits: reuse site's row layout; role muted, name cream (matches existing) */
  .story_credit-grid.cf-credits-built .cf-credit-role{font-family:"Hanken Grotesk",sans-serif;font-size:14.5px;font-weight:400;color:rgb(103,100,94)}
  .story_credit-grid.cf-credits-built .cf-credit-name{font-family:"Hanken Grotesk",sans-serif;font-size:14.5px;font-weight:400;color:rgb(243,239,230)}
  .story_credit-grid.cf-credits-empty{display:none!important}
  ...
  /* ===== hero gallery (v27): image stepper replaces the hero video on no-video studies ===== */
  .cf-hero-built{aspect-ratio:auto!important;height:auto!important;padding-top:0!important;overflow:visible!important;background:transparent!important}
  .cf-hero-built>.case-study_video,.cf-hero-built>iframe,.cf-hero-built>video,.cf-hero-built>.cf-cover,.cf-hero-built>.cf-spin{display:none!important}
  .cf-hero-built .cf-gal-track{padding:0 6%}
  .cf-hero-built .cf-gal-cell{flex:0 0 86%;aspect-ratio:16/9}
  @media(max-width:600px){
    .cf-hero-built .cf-gal-track{padding:0}
    .cf-hero-built .cf-gal-cell{flex:0 0 100%}
  }
  ...
  .story_slider-wrap-main.cf-gallery-empty{display:none!important}
  ...
  // Hero gallery (v27): one "image :: url" per line. Image-only; captions ignored.
  const parseHero = raw => {
    if (!raw) return [];
    return raw.split(/\r?\n/).map(line => {
      const t = line.trim();
      if (!t) return null;
      const parts = t.split(' :: ');
      if (parts.length < 2) { console.warn('[cf-hero] skipped (need "image :: url"):', t); return null; }
      const type = parts[0].trim().toLowerCase();
      const ref = parts[1].trim();
      if (type !== 'image') { console.warn('[cf-hero] skipped (images only, got "' + type + '"):', t); return null; }
      if (!ref) { console.warn('[cf-hero] skipped (no url):', t); return null; }
      return { type: 'image', ref, caption: '' }; // captions deliberately ignored
    }).filter(Boolean);
  };

  // Credits: one "Role :: Name" per line (e.g. "Director :: Dom Megna")
  const parseCredits = raw => {
    ...
    return raw.split(/\r?\n/).map(line => {
      ...
      if (!t) return null;
      const parts = t.split(' :: ');
      if (parts.length < 2) { console.warn('[cf-credits] skipped (need "Role :: Name"):', t); return null; }
  ...
  // Meta: one "Label :: Value" per line (e.g. "Client :: Autobarn"). Same delimiter as credits.
  const parseMeta = raw => {
    if (!raw) return [];
    return raw.split(/\r?\n/).map(line => {
      const t = line.trim();
      if (!t) return null;
      const parts = t.split(' :: ');
      if (parts.length < 2) { console.warn('[cf-meta] skipped (need "Label :: Value"):', t); return null; }
  ...
  const makeCell = (it, realI, opts = {}) => {
    ...
    tagOrient(img, cell);
    if (it.caption && !opts.noCaption) { const c = document.createElement('div'); c.className = 'cf-gal-cap'; c.textContent = it.caption; cell.appendChild(c); }
    return cell;
  };

  // In-popup gallery: transform-stepper with seamless loop (ported from mockup).
  const buildStepper = (wrap, items, opts = {}) => {
    ...
    const CLONES = Math.min(2, N); // v27 fix: 2 fixed clones crashed 1-item galleries (items[1] undefined)
    const layout = [];
    for (let i = N - CLONES; i < N; i++) layout.push((i + N) % N);
    for (let i = 0; i < N; i++) layout.push(i);
    for (let i = 0; i < CLONES; i++) layout.push(i);
    layout.forEach(real => track.appendChild(makeCell(items[real], real, opts)));
    ...
    // click a cell -> open THIS stepper's lightbox (unless it was a swipe-drag)
    const myItems = items.slice(); // instance-owned feed — hero & bottom galleries share no state
    const onCellClick = ev => { const cell = ev.target.closest('.cf-gal-cell'); if (!cell || dragMoved) return; openDialog(myItems, +cell.dataset.real, true); };
  ...
  /* ---------------- hero gallery (v27) ----------------
     If a case study has NO real hero video, [cf-hero-data] images become a
     stepper in .story_video-wrap. A video ALWAYS wins: a vimeoMap entry (the
     engine will mount its own player) or any iframe/video with a non-empty
     src — or data-src, since stripVideo() parks native srcs there. The
     .cf-cover poster clone is NOT a real video and is excluded. */
  const heroHasVideo = m => {
    if (vimeoMap[slugOf(m)]) return true;
    const scope = m.querySelector('.case-study_video') || m.querySelector('.story_video-wrap');
    if (!scope) return false;
    return [...scope.querySelectorAll('iframe,video,source')].some(el => {
      if (el.classList && el.classList.contains('cf-cover')) return false;
      if (el.closest && el.closest('.cf-cover')) return false;
      const s = (el.getAttribute('src') || (el.dataset && el.dataset.src) || '').trim();
      return !!s;
    });
  };

  const buildVisibleHero = () => {
    const m = activeModal();
    if (!m) return;
    const dataEl = m.querySelector('[cf-hero-data]');
    if (!dataEl) return; // no hero field on this study
    dataEl.style.display = 'none';
    if (heroHasVideo(m)) return; // video always wins — leave the hero alone
    const items = parseHero(dataEl.textContent);
    if (!items.length) { console.warn('[cf-hero] no valid items for', slugOf(m)); return; }
    const wrap = m.querySelector('.story_video-wrap') || heroWrap(m);
    if (!wrap) { console.warn('[cf-hero] no .story_video-wrap for', slugOf(m)); return; }
    wrap.classList.add('cf-hero-built');
    wrap.style.paddingTop = '0'; // mountHero may have set 56.25%
    // hide the videoless embed chrome (embed box, covers, spinners) but never our stepper
    [...wrap.children].forEach(ch => { if (!ch.classList.contains('cf-gal') && ch.matches('.case-study_video,iframe,video,.cf-cover,.cf-spin')) ch.style.display = 'none'; });
    buildStepper(wrap, items, { noCaption: true });
  };
  ...
  const buildVisibleGallery = () => {
    ...
    // Always mark built so the CSS that hides the OLD swiper/splide applies, even when
    // there are no items (otherwise the dead slider shows as a stray line + arrows).
    m.classList.add('cf-gallery-built');
    wrap.classList.add('cf-gallery-built');
    if (!items.length) {
      console.warn('[cf-gallery] no valid items for', slugOf(m));
      wrap.classList.add('cf-gallery-empty'); // hides the whole wrap via CSS
      return;
    }
    wrap.classList.remove('cf-gallery-empty');
    buildStepper(wrap, items);
  };

  // Build credits from a single [cf-credits-data] field, if present, into the existing
  // .story_credit-grid using the site's own row classes (so styling is inherited).
  const buildVisibleCredits = () => {
    const m = activeModal();
    if (!m) return;
    const dataEl = m.querySelector('[cf-credits-data]');
    if (!dataEl) return; // no delimited field on this study -> leave native credits alone
    const credits = parseCredits(dataEl.textContent);
    const grid = m.querySelector('.story_credit-grid');
    if (!grid) { console.warn('[cf-credits] no .story_credit-grid for', slugOf(m)); return; }
    if (!credits.length) { grid.classList.add('cf-credits-empty'); return; }
    grid.classList.remove('cf-credits-empty');
    grid.innerHTML = '';
    credits.forEach(c => {
      const row = document.createElement('div'); row.className = 'story_credit-row cf-credit-row';
      const role = document.createElement('div'); role.className = 'story_credit-role cf-credit-role'; role.textContent = c.role;
      const name = document.createElement('div'); name.className = 'story_credit-name cf-credit-name'; name.textContent = c.name;
      row.append(role, name); grid.appendChild(row);
    });
    grid.classList.add('cf-credits-built');
  };

  // Build meta bar from a single [cf-meta-data] field, if present, into the existing
  // .story_item-info-wrap using the site's own classes (so styling + 4-across layout inherit).
  const buildVisibleMeta = () => {
    const m = activeModal();
    if (!m) return;
    const dataEl = m.querySelector('[cf-meta-data]');
    if (!dataEl) return; // no delimited field -> leave native meta fields alone
    const items = parseMeta(dataEl.textContent);
    const wrap = m.querySelector('.story_item-info-wrap');
    if (!wrap) { console.warn('[cf-meta] no .story_item-info-wrap for', slugOf(m)); return; }
    if (!items.length) { wrap.classList.add('cf-meta-empty'); return; }
    wrap.classList.remove('cf-meta-empty');
    wrap.innerHTML = '';
    items.forEach(it => {
      const pair = document.createElement('div'); pair.className = 'story_cs-meta cf-meta-pair';
      const label = document.createElement('div'); label.className = 'story_item-info-lable cf-meta-label'; label.textContent = it.label;
      const value = document.createElement('div'); value.className = 'story_item-info-lable text cf-meta-value'; value.textContent = it.value;
      pair.append(label, value); wrap.appendChild(pair);
    });
    wrap.classList.add('cf-meta-built');
  };

  const initVisibleSliders = () => { buildVisibleHero(); buildVisibleGallery(); buildVisibleCredits(); buildVisibleMeta(); };
  ...
  document.addEventListener('click', e => {
    ...
    // --- a lightbox is open: it owns all clicks. Never fall through to popup close. ---
    if (lbStack.length) {
      if (e.target.closest('.csvd_x') || e.target.closest('.csvd_nav')) { /* handled by the dialog's own listeners */ return; }
  ...
  document.addEventListener('keydown', e => {
    ...
    if (top) {
      ...
      else if (e.key === 'ArrowRight') { top.go(1); }
      else if (e.key === 'ArrowLeft') { top.go(-1); }
      return;
    }
  ...
  /* csvd lightbox — desktop: arrows + one item. mobile: scroll-snap strip, peek + dots.
     v27: every open dialog is a self-contained instance on lbStack (last =
     topmost). Instances own their items/track/pos/anim/CLONES and their
     place/sync/go/goTo/close. Body scroll lock is NEVER released on lightbox
     close — the case-study popup beneath still owns it (closePopup unlocks). */
  const lbStack = [];

  function closeDialog() { const top = lbStack[lbStack.length - 1]; if (top) top.close(); }
  ...
  function openDialog(items, index, autoplay) {
    items = (items || []).slice(); // instance-owned copy — no shared state
    ...
    const track = document.createElement('div'); track.className = 'csvd_track'; vp.appendChild(track);
    ...
    for (let i = 0; i < N; i++) layout.push(i);
    for (let i = 0; i < CLONES; i++) layout.push(i);
    layout.forEach(real => { const cell = document.createElement('div'); cell.className = 'csvd_cell'; cell.dataset.real = real; fillCell(cell, items[real], false, false); track.appendChild(cell); });
    ...
    lockBody(true); // popup beneath already holds the lock; close() never releases it

    let pos = CLONES + (index || 0), anim = false;
    const realOf = i => +track.children[i].dataset.real;
    const offsetFor = i => { const c = track.children[i]; const vpW = vp.clientWidth; return -(c.offsetLeft - (vpW - c.offsetWidth) / 2); };
    const place = animate => { track.style.transition = animate ? 'transform .5s cubic-bezier(.22,.61,.36,1)' : 'none'; track.style.transform = 'translateX(' + offsetFor(pos) + 'px)'; };
    const sync = () => {
      const real = realOf(pos);
      ...
      dots.querySelectorAll('.csvd_dot').forEach((dot, i) => dot.classList.toggle('is-active', i === real));
    };
    const go = dir => {
      if (anim) return; anim = true;
      let target = pos + dir;
      if (target < CLONES) { place(false); pos += N; place(false); void track.offsetWidth; target = pos + dir; }
      else if (target >= CLONES + N) { place(false); pos -= N; place(false); void track.offsetWidth; target = pos + dir; }
      pos = target; place(true); sync();
    };
    const goTo = realIdx => { if (anim) return; anim = true; pos = CLONES + realIdx; place(true); sync(); };
    const close = () => {
      const k = lbStack.indexOf(inst);
      if (k >= 0) lbStack.splice(k, 1);
      d.remove();
      // deliberately NOT lockBody(false): the case-study popup is still open beneath
    };

    // make the initial cell live + autoplay if user-initiated
    fillCell(track.children[pos], items[realOf(pos)], true, !!autoplay);
    track.children[pos].classList.add('is-live');

    const onEnd = e => { if (e.propertyName === 'transform') anim = false; };
    track.addEventListener('transitionend', onEnd);
    x.addEventListener('click', ev => { ev.stopPropagation(); close(); });
    prev.addEventListener('click', ev => { ev.stopPropagation(); go(-1); });
    next.addEventListener('click', ev => { ev.stopPropagation(); go(1); });
    dots.addEventListener('click', ev => { const dot = ev.target.closest('.csvd_dot'); if (!dot || anim) return; goTo(+dot.dataset.i); });
    d.addEventListener('click', ev => { if (!ev.target.closest('.csvd_vp,.csvd_dots,.csvd_nav,.csvd_x')) close(); });
    ...
    const lbTe = e => { if (!sw) return; sw = false; if (axis !== 'x') return; const dx = e.changedTouches[0].clientX - sx; if (dx < -45) go(1); else if (dx > 45) go(-1); };
    ...
    const inst = { el: d, go, goTo, close };
    d._lb = inst;
    lbStack.push(inst);

    requestAnimationFrame(() => { place(false); sync(); });
    return inst;
  }

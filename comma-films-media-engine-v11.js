/* ============================================================
   Comma Films — home-new media engine v11
   v11 changes:
...
(() => {
  ...
  @media(max-width:600px){.csvd_swipe{display:block;position:fixed;left:0;right:0;top:calc(50% - 28vw);height:56vw;z-index:999999}}
  ...
  @media(max-width:600px){
    ...
    /* keep X at the very top, clear of the centered media */
    .csvd_x{top:max(14px,env(safe-area-inset-top));right:16px}
    /* arrows vertically centered on the media, not screen-bottom */
    .csvd_nav{top:50%;bottom:auto;transform:translateY(-50%);width:42px;height:42px}
    .csvd_nav:hover{transform:translateY(-50%) scale(1.05)}
    .csvd_prev{left:12px}.csvd_next{right:12px}
    /* caption sits just under the centered media box */
    .csvd_cap{position:absolute;left:16px;right:16px;bottom:auto;top:calc(50% + 30vw);text-align:center}
  }
  /* touch devices: no sticky hover — only show red on actual press, not after tap */
  @media(hover:none){
    .csvd_nav:hover,.csvd_x:hover,.story_slider-wrap-main .splide__arrow--prev:hover,.story_slider-wrap-main .splide__arrow--next:hover{background:rgba(11,11,12,.7)!important;border-color:#ffffff2e!important;transform:translateY(-50%)}
    ...
    .csvd_x:active{background:rgb(232,90,79)!important}
    .csvd_x:active:before,.csvd_x:active:after{background:#000!important}
  }

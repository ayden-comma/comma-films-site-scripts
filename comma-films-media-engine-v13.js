/* ============================================================
   Comma Films — home-new media engine v13
   v13 changes:
...
(() => {
  ...
  /* mobile: center the whole lightbox; X pinned top; arrows flank the media */
  /* MOBILE lightbox: horizontal scroll-snap strip, active centered, neighbors peek, dots */
  .csvd_strip{display:none}
  .csvd_dots{display:none}
  .csvd_peekmedia{position:relative;width:100%;height:100%}
  .csvd_peekmedia img{width:100%;height:100%;object-fit:contain;display:block}
  @media(max-width:600px){
    .csvd{flex-direction:column;align-items:stretch;justify-content:center;padding:0}
    .csvd_b,.csvd_nav{display:none!important}
    .csvd_x{top:max(14px,env(safe-area-inset-top));right:16px}
    .csvd_strip{display:flex;gap:12px;width:100vw;overflow-x:auto;scroll-snap-type:x mandatory;-webkit-overflow-scrolling:touch;scrollbar-width:none;padding:0 12vw}
    .csvd_strip::-webkit-scrollbar{display:none}
    .csvd_cell{flex:0 0 76vw;scroll-snap-align:center;height:auto;aspect-ratio:16/10;border-radius:12px;overflow:hidden;background:#000;position:relative;display:flex;align-items:center;justify-content:center;opacity:.5;transition:opacity .3s}
    .csvd_cell.is-live{opacity:1}
    .csvd_cell img,.csvd_cell iframe{width:100%;height:100%;object-fit:cover;display:block;border:0}
    .csvd_cell .cf-gal-play{position:absolute;inset:0;margin:auto;width:52px;height:52px}
    .csvd_cap{position:static;margin:18px auto 10px;left:auto;right:auto;bottom:auto;top:auto;text-align:center;line-height:1.3;padding:0 24px}
    .csvd_dots{display:flex;justify-content:center;gap:7px;margin-bottom:max(24px,env(safe-area-inset-bottom))}
    .csvd_dot{width:7px;height:7px;border-radius:50%;background:#ffffff45;transition:background .25s,transform .25s}
    .csvd_dot.is-active{background:rgb(232,90,79);transform:scale(1.25)}
  }
  /* touch: no sticky hover on desktop-path arrows (still used >600px on touch laptops) */
  @media(hover:none){
    .csvd_nav:hover,.story_slider-wrap-main .splide__arrow--prev:hover,.story_slider-wrap-main .splide__arrow--next:hover{background:rgba(11,11,12,.7)!important;border-color:#ffffff2e!important}
    .csvd_nav:hover svg,.story_slider-wrap-main .splide__arrow--prev:hover svg,.story_slider-wrap-main .splide__arrow--next:hover svg,.story_slider-wrap-main .splide__arrow--prev:hover svg path,.story_slider-wrap-main .splide__arrow--next:hover svg path{stroke:#fff!important;fill:none!important}
    ...
    .csvd_nav:active{background:rgb(232,90,79)!important;border-color:rgb(232,90,79)!important}
    .csvd_nav:active svg{stroke:#000!important}
  }
  ...
  function openDialog(index, autoplay) {
    ...
    if (isMobileLB() && currentGalleryItems.length > 0) {
      // MOBILE: scroll-snap strip + dots, no arrows

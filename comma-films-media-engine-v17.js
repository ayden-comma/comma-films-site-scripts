/* ============================================================
   Comma Films — home-new media engine v17
   v16 changes:
...
(() => {
  ...
  .cf-gal-cell{flex:0 0 67%;position:relative;aspect-ratio:16/10;border-radius:10px;overflow:hidden;background:#000;border:1px solid #ffffff14;cursor:pointer;opacity:.55;transition:opacity .35s cubic-bezier(.22,.61,.36,1)}
  @media(max-width:600px){.cf-gal-cell{flex:0 0 90%}}
  .cf-gal-cell.is-active{opacity:1}
  .cf-gal-cell img.cf-gal-thumb{width:100%;height:100%;object-fit:contain;display:block;filter:brightness(.9);transition:transform .8s cubic-bezier(.22,.61,.36,1),filter .55s}
  ...
  const buildStepper = (wrap, items) => {
    ...
    // swipe (mobile)
    let sx = 0, sw = false;
    const ts = e => { sx = e.touches[0].clientX; sw = true; dragMoved = false; };
    const tm = e => { if (sw && Math.abs(e.touches[0].clientX - sx) > 10) dragMoved = true; };
    const te = e => { if (!sw) return; sw = false; const dx = e.changedTouches[0].clientX - sx; if (dx < -45) go(1); else if (dx > 45) go(-1); };
  ...
  function openDialog(index, autoplay) {
    ...
    // swipe
    let sx = 0, sw = false;
    d.addEventListener('touchstart', e => { sx = e.touches[0].clientX; sw = true; }, { passive: true });
    d.addEventListener('touchend', e => { if (!sw) return; sw = false; const dx = e.changedTouches[0].clientX - sx; if (dx < -45) lbGo(1); else if (dx > 45) lbGo(-1); }, { passive: true });

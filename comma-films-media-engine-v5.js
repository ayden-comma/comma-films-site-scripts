/* ============================================================
   Comma Films — home-new media engine v5
   v5 changes:
...
(() => {
  ...
  /* rebuilt gallery (v5) — transform-stepper, ported from case-study mockup */
  [cf-gallery-data]{display:none!important}
  .cf-gal{position:relative;margin-top:8px}
  .cf-gal-nav{position:absolute;top:-54px;right:0;display:flex;gap:8px;z-index:3}
  .cf-gal-btn{width:44px;height:44px;border-radius:50%;cursor:pointer;background:rgba(20,18,16,.6);border:1px solid #ffffff2e;display:grid;place-items:center;transition:background .25s,transform .25s cubic-bezier(.22,.61,.36,1),border-color .25s}
  .cf-gal-btn:hover{background:var(--accent,#c8833a);border-color:var(--accent,#c8833a);transform:translateY(-2px)}
  .cf-gal-btn:hover svg{stroke:#000}
  .cf-gal-btn svg{width:16px;height:16px;stroke:#f2ede6;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round;fill:none;transition:stroke .25s}
  .cf-gal-viewport{overflow:hidden;border-radius:10px}
  .cf-gal-track{display:flex;gap:14px;will-change:transform}
  .cf-gal-slide{position:relative;flex:0 0 auto;width:calc((100% - 14px)/2);aspect-ratio:16/10;border-radius:10px;overflow:hidden;background:#141210;border:1px solid #ffffff14}
  @media(max-width:600px){.cf-gal-slide{width:100%}}
  .cf-gal-slide img.cf-gal-thumb{width:100%;height:100%;object-fit:cover;display:block;transition:transform .8s cubic-bezier(.22,.61,.36,1),filter .55s}
  .cf-gal-slide.is-media:hover img.cf-gal-thumb{transform:scale(1.05)}
  .cf-gal-slide .cf-gal-play{position:absolute;inset:0;margin:auto;width:56px;height:56px;border-radius:50%;background:rgba(12,12,14,.5);border:1px solid #ffffff42;display:flex;align-items:center;justify-content:center;pointer-events:none;z-index:2;transition:opacity .3s}
  ...
  .cf-gal-slide .cf-gal-cap{position:absolute;left:0;right:0;bottom:0;z-index:3;padding:30px 16px 13px;font-family:"Red hat display",sans-serif;font-size:14px;letter-spacing:.01em;color:#f2ede6;background:linear-gradient(to top,rgba(8,8,9,.92),rgba(8,8,9,.55) 40%,transparent)}
  .cf-gallery-built .story_slider-wrap-main > .w-dyn-list,.cf-gallery-built .splide:not(.cf-gal){display:none!important}
  ...
  const makeSlide = it => {
    ...
    s.className = 'cf-gal-slide' + (it.type !== 'image' ? ' is-media' : '');
  ...
  // Build a full stepper into the gallery wrap, replacing whatever's there.
  const buildStepper = (wrap, items) => {
    ...
    const nav = document.createElement('div'); nav.className = 'cf-gal-nav';
    const prev = document.createElement('button'); prev.className = 'cf-gal-btn'; prev.type = 'button'; prev.setAttribute('aria-label', 'Previous'); prev.innerHTML = arrowSvg('prev');
    const next = document.createElement('button'); next.className = 'cf-gal-btn'; next.type = 'button'; next.setAttribute('aria-label', 'Next'); next.innerHTML = arrowSvg('next');
    ...
    // clone for wrap only when there's something to wrap through
    if (n > 1) items.forEach(it => { const c = makeSlide(it); c.setAttribute('aria-hidden', 'true'); track.appendChild(c); });
    vp.appendChild(track); gal.append(nav, vp); wrap.appendChild(gal);
    ...
    setX(0, false);
    galleries.set(wrap, { teardown: () => { track.removeEventListener('transitionend', onEnd); prev.removeEventListener('click', onPrev); next.removeEventListener('click', onNext); } });
  };
